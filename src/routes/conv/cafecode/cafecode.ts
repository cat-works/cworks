...
function parseFloat(integer: number): number {
...
class AssemblyExecuteStmt extends Stmt {
  ...
  constructor(machine_codes: number[]) {
    ...
    try {
      if (d == undefined) {
        d = new cs.Capstone(cs.ARCH_PPC, cs.MODE_BIG_ENDIAN);
      }
...
class TimerStmt extends BlockStmt {
  constructor(protected frames: number, body: Stmt[]) {
    super(body);
  }
...
export default class Parser {
  ...
  constructor(_src: string) {
    ...
    */
    ...
    let byte_strs = src.split(/(..)/).slice(1).filter(x => x.length == 2);
    this.src = byte_strs.map(x => parseInt(x, 16));
  }
  read_byte(): number {
    let ret = this.src.shift();
    if (!ret) {
      return 0;
    }
  ...
  startsWith(bytes: Array<number>) {
    return this.src.
      slice(0, bytes.length).
      every((x, i) => x == bytes[i]);
  }

  parseIf(opcode: number): IfNode {
    ...
    let size = this.convert_size(byte & 0x0f);
    if (size == null) {
      throw new Error("Invalid size: " + (byte & 0x0f));
    }
  ...
  parseRamWrite() {
    ...
    if (size == null) {
      throw new Error("Invalid size: " + (ps & 0x0f));
    }
  ...
  parseRegisterAssign(opcode: number) {
  ...
  parseRegisterOperator(opcode: number) {
  ...
  parseStmts(sentinel: number[] | undefined = undefined) {
